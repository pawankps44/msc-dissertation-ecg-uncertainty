...
OUT=os.path.expanduser("~/protoecgnet/experiments/test_results/calibration_eval_chapman_s123"); os.makedirs(OUT,exist_ok=True)
MODELS={"standard":"chapman_standard_s123_clf","bce_weighted":"chapman_bce_s123_clf"}
...
_, val_loader, test_loader, _ = get_chapman_dataloaders(batch_size=32, work_num=4, return_sample_ids=False,seed=123)
...
def load_model(job):
    ...
    m=ProtoECGNet1D(num_classes=num_classes,single_class_prototype_per_class=6,joint_prototypes_per_border=0,
        ...
        custom_groups=True,label_set="chapman",pretrained_weights=ck).to(DEVICE); m.eval(); return m
...
df=pd.DataFrame(rows); df.to_csv(os.path.join(OUT,"chapman_calibration.csv"),index=False)
