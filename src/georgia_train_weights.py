...
T_MC=30
CKPT_DIR=os.path.expanduser("~/protoecgnet/experiments/checkpoints/georgia_standard_clf")
...
train_loader,_,_,_=get_georgia_dataloaders(batch_size=32, work_num=4, return_sample_ids=True)
model=ProtoECGNet1D(num_classes=num_classes, single_class_prototype_per_class=6,
    ...
    custom_groups=True, label_set="georgia", pretrained_weights=ckpt).to(DEVICE)
model.eval()
ids_all=[]
with torch.no_grad():
...
np.savez(os.path.join(OUT,"georgia_train_weights.npz"), ecg_id=uids, uncertainty=unc, weight=weight)
print("weight stats -> min %.3f mean %.3f max %.3f"%(weight.min(),weight.mean(),weight.max()))
print("saved", os.path.join(OUT,"georgia_train_weights.npz"))
