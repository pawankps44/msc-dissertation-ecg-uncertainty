...
cd ~/protoecgnet/src
SEED=$1; MODE=$2
TAG=chapman_${MODE}_s${SEED}
CK=$HOME/protoecgnet/experiments/checkpoints
BASE="--dimension 1D --backbone resnet1d18 --sampling_rate 100 --label_set chapman --custom_groups True --dataset chapman --dropout 0.35 --num_workers 4 --save_weights True --checkpoint_dir $CK --log_dir $HOME/protoecgnet/experiments/logs --test_dir $HOME/protoecgnet/experiments/test_results --seed $SEED"
PROTO="--single_class_prototype_per_class 6 --joint_prototypes_per_border 0 --proto_time_len 32 --proto_dim 512"
WF=""
if [ "$MODE" = "bce" ]; then WF="--sample_weights_path $HOME/protoecgnet/experiments/preprocessing/chapman_train_weights.npz --weight_bce True"; fi
