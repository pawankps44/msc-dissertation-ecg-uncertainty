#!/bin/bash
#SBATCH -J cat1_proto_classifier
#SBATCH -c4 --mem=16G -G1
#SBATCH -p general
source ~/ecg_venv/bin/activate
cd ~/protoecgnet/src
python main.py \
  --job_name cat1_proto_classifier \
  --epochs 100 \
  --batch_size 32 \
  --lr 0.0001 \
  ...
  --seed 42 \
  --pretrained_weights "$HOME/protoecgnet/experiments/checkpoints/cat1_proto_proj/cat1_proto_proj_projection.pth"