#!/bin/bash
#SBATCH -J cat1_proj_wclst
#SBATCH -c4 --mem=16G -G1
#SBATCH -p general
source ~/ecg_venv/bin/activate
cd ~/protoecgnet/src
python main.py \
  --job_name cat1_proto_proj_wclst \
  --epochs 1 \
  --batch_size 32 \
  ...
  --seed 42 \
  --pretrained_weights "$HOME/protoecgnet/experiments/checkpoints/cat1_proto_joint_wclst/epoch=18-val_auc=0.8493.ckpt"
