#!/bin/bash
#SBATCH -J cat1_proj_wcs
#SBATCH -c4 --mem=16G -G1
#SBATCH -p general
source ~/ecg_venv/bin/activate
cd ~/protoecgnet/src
python main.py \
  --job_name cat1_proto_proj_wcs \
  --epochs 1 \
  --batch_size 32 \
  ...
  --seed 42 \
  --pretrained_weights "$HOME/protoecgnet/experiments/checkpoints/cat1_proto_joint_wcs/epoch=18-val_auc=0.8442.ckpt"
